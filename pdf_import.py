...
def extract_from_pdf(pdf_bytes: bytes):
    ...
    result["Result of analysis"] = _search_text(all_text, [r"(?:Evaluation|Fault Codes?).*?\b(T[1-3]|PD|D[1-2]|DT|N)\b", r"Result\s*of\s*analysis\s*[:\-]?\s*(.+)", r"\b(T[1-3]|PD|D[1-2]|DT|N)\b"])
    result["DGA"] = _search_text(all_text, [r"Detection.*?\b(S[1-3])\b", r"\bDGA\s*[:\-]?\s*([A-Z0-9\-]+)", r"\b(S[1-3])\b"])
    result["C.Recommended"] = _search_text(all_text, [r"Recommended.*?\b(R[1-9M]|A|I|FAL)\b", r"Recommended\s*[:\-]?\s*(R[1-9])", r"التوصية\s*[:\-]?\s*(R[1-9])", r"\b(R[1-9])\b"])
    return result
