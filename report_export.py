...
def generate_sample_pdf(row: dict):
    ...
    c.setFillColor(colors.HexColor("#1f4e79")); c.rect(0,h-2.2*cm,w,2.2*cm,stroke=0,fill=1)
    c.setFillColor(colors.white); c.setFont("Helvetica-Bold",16); c.drawString(2*cm,h-1.3*cm,"Transformer Oil DGA Report")
    c.setFont("Helvetica",9); c.drawRightString(w-1.5*cm,h-1.1*cm,dt.datetime.now().strftime("%Y-%m-%d %H:%M"))
    y=h-3.2*cm; c.setFillColor(colors.black); c.setFont("Helvetica-Bold",12); c.drawString(2*cm,y,"Asset & Sample Info"); y-=0.5*cm; c.setFont("Helvetica",10)
    def line(lbl,key): nonlocal y; c.drawString(2*cm,y,f"{lbl}: {row.get(key,'')}"); y-=0.6*cm
    ...
    x1,x2=2*cm,10*cm
    for g_label,key in gases: c.drawString(x1,y,f"{g_label}:"); c.drawString(x2,y,f"{row.get(key,'')}"); y-=0.5*cm
