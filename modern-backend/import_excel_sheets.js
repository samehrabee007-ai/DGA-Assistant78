...
async function run() {
    ...
    // Loop over sheets 1 and 2 (المنيا and اسيوط الوادي)
    for(let s = 1; s < 3; s++) {
        const ws = wb.worksheets[s];
        console.log(`\nProcessing Sheet ${s} (${ws.name})...`);
        ...
        console.log(`Found ${rowsToInsert.length} valid rows to insert in sheet ${ws.name}.`);
        ...
        console.log(`Successfully inserted ${successCount} rows from sheet ${ws.name}.`);
    }
    
    console.log(`\nAll done! Total inserted from both sheets: ${totalSuccessCount} rows.`);
}
